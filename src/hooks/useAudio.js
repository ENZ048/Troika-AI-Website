...
export const useAudio = (isMuted, hasUserInteracted = true, onAudioEnded = null) => {
    ...
    (audioData, messageIndex) => {
      ...
      try {
        ...
        if (hasUserInteracted) {
          ...
          newAudio.muted = isMuted;
          console.log(`Applied mute state ${isMuted} before play`);
          ...
          if (playPromise !== undefined) {
            playPromise
              .then(() => {
                ...
                newAudio.muted = isMuted;
                console.log(`Confirmed mute state ${isMuted} after play started`);
              })
              .catch((err) => {
          ...
          newAudio.muted = isMuted;
          console.log(`Applied mute state ${isMuted} to prepared audio`);
        }
  ...
  const muteCurrentAudio = useCallback((mutedState) => {
    console.log("muteCurrentAudio called with state:", mutedState, "currentlyPlaying:", currentlyPlaying);
    const audioToMute = currentAudioRef.current || audioObject;
    if (audioToMute && currentlyPlaying !== null) {
      audioToMute.muted = mutedState;
      console.log(`Audio muted state set to: ${mutedState} for message ${currentlyPlaying}`);
      return true;
    }
    console.log("No audio currently playing to mute");
    return false;
  }, [audioObject, currentlyPlaying]);
  ...
  const ensureAudioMuted = useCallback((audioElement, mutedState) => {
    if (audioElement) {
      ...
      console.log(`Ensured audio muted state: ${mutedState}`);
    }
  }, []);
  ...
  useEffect(() => {
    if (audioObject && currentlyPlaying !== null) {
      console.log(`Ensuring mute state ${isMuted} is applied to audio for message ${currentlyPlaying}`);
