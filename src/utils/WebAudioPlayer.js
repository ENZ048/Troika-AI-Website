...
export class WebAudioPlayer {
  ...
  scheduleAudioChunk(pcmData) {
    try {
      ...
      console.log(`⏱️  Scheduled chunk at ${startTime.toFixed(3)}s, duration: ${duration.toFixed(3)}s, next: ${this.nextStartTime.toFixed(3)}s`);
  ...
  finalizeStream() {
    ...
    if (this.chunkBuffer.size > 0) {
      console.warn(`⚠️  ${this.chunkBuffer.size} audio chunks were out of order and not played`);
      console.warn('Missing sequences:', Array.from(this.chunkBuffer.keys()));
      this.chunkBuffer.clear();
    }

    console.log(`✅ Audio stream finalized. ${this.scheduledBuffers.length} chunks scheduled for playback`);
  }
   ...
   */
  destroy() {
    console.log('🗑️  Destroying WebAudioPlayer...');
